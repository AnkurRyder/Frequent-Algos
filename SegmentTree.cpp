...
int MinSTree(vector <int> arr, int ss, int se, vector <int> &st, int si)
{
	...
	int mid = (ss+ se)/2;
	st[si] = minVal(MinSTree(arr, ss, mid, st, si*2+1) , MinSTree(arr, mid+1, se, st, si*2+2));
	return st[si];
}

int getMin(std::vector<int> &st, int ss, int se, int qs, int qe, int index)
{
	...
	int mid = (ss+ se)/2;
	return minVal( getMin(&st, ss, mid, qs, qe, 2*index+1) , getMin(&st, mid+1, se, qs, qe, 2*index+2));
}
...
int updateMinValue(vector <int> arr[], vector <int> &st, int ss, int se, int i, int new_val, int si)
	...
	int mid = (ss+ se)/2;
	st[si] = minVal(updateMinValue(arr, st, ss, mid, i, new_val, 2*si+ 1) , updateMinValue(arr, st, mid+1, se, i, new_val, 2*si+ 2));
	return st[si];
}